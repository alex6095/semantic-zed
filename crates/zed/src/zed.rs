...
use semantic_zed::{PaperPanel, PdfPreviewPanel};
use settings::{
...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let paper_panel = PaperPanel::load(workspace_handle.clone(), cx.clone());
        let pdf_preview_panel = PdfPreviewPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            ...
            add_panel_when_ready(paper_panel, workspace_handle.clone(), cx.clone()),
            add_panel_when_ready(pdf_preview_panel, workspace_handle.clone(), cx.clone()),
